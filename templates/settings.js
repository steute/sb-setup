...
// Passwords are retrieved from docker secrets, which are mapped to files in the container.
// But there is also a fallback to environment variables. (Should be used for development only)
// For more details on how to use secrets, refer to the documentation in CONFIGURATION.md

const fs = require("fs");

var secretNodeRedAdminPassword = false;
if (
  process.env.NODE_RED_ADMIN_PASSWORD_FILE &&
  fs.existsSync(process.env.NODE_RED_ADMIN_PASSWORD_FILE)
) {
...
var secretNodeRedReadOnlyPassword = false;
if (
  process.env.NODE_RED_READONLY_PASSWORD_FILE &&
  fs.existsSync(process.env.NODE_RED_READONLY_PASSWORD_FILE)
) {
...
var secretNodeRedCredentialSecret = false;
if (
  process.env.NODE_RED_CREDENTIAL_SECRET_FILE &&
  fs.existsSync(process.env.NODE_RED_CREDENTIAL_SECRET_FILE)
) {
...
// Workaround to have a string "false" instead of a boolean false
if (secretNodeRedCredentialSecret == "false") {
  secretNodeRedCredentialSecret = false;
}

// This is the path or sub-path where the Node-RED editor and dashboard will be available.
// Will be used to set the httpAdminRoot and httpNodeRoot properties.
// Also important for the nginx reverse proxy configuration.
// Has to start with a slash and should not end with a slash.
let nodeRedRootPath = "/nodered";
if (process.env.NODE_RED_ROOT_PATH) {
  // ad leading slash if not present
  nodeRedRootPath = process.env.NODE_RED_ROOT_PATH.startsWith("/")
    ? process.env.NODE_RED_ROOT_PATH
    : "/" + process.env.NODE_RED_ROOT_PATH;
  // remove trailing slash if present
  nodeRedRootPath = nodeRedRootPath.endsWith("/")
...
module.exports = {
   ...
   * lost.
   *
   * This setting will take the secret from a docker secret.
   * The file containing the secret should be mounted to the container at
   * process.env.NODE_RED_CREDENTIAL_SECRET_FILE
   */
  credentialSecret: secretNodeRedCredentialSecret || false,
   ...
   *
   * Default user is admin and password is steute.
   */
  adminAuth: {
    type: "credentials",
    sessionExpiryTime: 14400, // 14400 = 4 hours
    users: [
      {
        username: process.env.NODE_RED_ADMIN_USERNAME || "admin",
        password: secretNodeRedAdminPassword,
        permissions:
          process.env.NODE_RED_ENABLE_ADMIN_USER == "true" ? "*" : "",
      },
      {
        username: process.env.NODE_RED_READONLY_USERNAME || "readonly",
        password: secretNodeRedReadOnlyPassword,
        permissions:
          process.env.NODE_RED_ENABLE_READONLY_USER == "false" ? "" : "read",
      },
    ],
  },
   ...
   */
  httpAdminRoot: nodeRedRootPath + "/admin",
   ...
   */
  disableEditor: process.env.NODE_RED_ENABLE_EDITOR == "true" ? false : true,
